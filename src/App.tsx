...
function DriverDashboard({ user, onNavigate, onAction }: { user: AppUser; onNavigate: (page: PageKey) => void; onAction: (message: string) => void }) {
  return <>
    ...
    <div className="role-dashboard-grid">
      ...
      <section className="panel quick-panel"><div className="panel-header"><div><h2>Akcije vozača</h2><p>Brze potvrde sa terena</p></div></div><div className="quick-action-grid driver-actions"><button onClick={() => onNavigate("deliveries")}><PackageCheck size={18} /><span>Potvrdi isporuku</span><small>Potpis i količina</small></button><button onClick={() => onAction("Otvoren je obrazac za prijavu povrata")}><ArrowDownToLine size={18} /><span>Prijavi povrat</span><small>LOT i razlog</small></button><button onClick={() => onNavigate("vehicles")}><Gauge size={18} /><span>Nova kontrola</span><small>Temperatura vozila</small></button><button onClick={() => onNavigate("tasks")}><ListChecks size={18} /><span>Novi zadatak</span><small>Moje obaveze</small></button><button onClick={() => onAction("Podrška distribucije je obaveštena")}><Bell size={18} /><span>Pozovi dispečera</span><small>Operativna podrška</small></button></div><div className="driver-note"><ShieldCheck size={16} /><span>Vozilo je spremno za nastavak rute. Sledeća obavezna kontrola: po povratku.</span></div></section>
    </div>
